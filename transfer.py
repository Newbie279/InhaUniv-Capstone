...
def getrequest():
    ...
    name = mykey[0]
    attr1 = result3[mykey[0]]['Attr1']
    attr2 = result3[mykey[0]]['Attr2']
    print('Attr1 is ',attr1,' And Attr2 is ',attr2)
    print(type(attr1),type(attr2))
    return name , attr1, attr2

def updatedb(name , attr1 , attr2):
...
def listener():
    cond = True
    while cond :
        name , attr1 ,attr2 =getrequest()
        if name ==None:
            continue
        else:
            updatedb(name,attr1,attr2)
            cond = False
