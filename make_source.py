...
def make_source(sourcenum=2,filename='videoplayback',image_num=1000):

    save_dir = Path('./data/source/'+str(sourcenum)+'/')
    save_dir.mkdir(exist_ok=True)

    img_dir = save_dir.joinpath('images')
    img_dir.mkdir(exist_ok=True)

    if len(os.listdir('./data/source/'+str(sourcenum)+'/images'))<100:
        cap = cv2.VideoCapture(str(save_dir.joinpath(filename)))
        i = 0
        while (cap.isOpened()):
            flag, frame = cap.read()
            if flag == False or i >= image_num:
                break
            #cv2.imwrite(str(img_dir.joinpath('{:05}.png'.format(i))), frame)
            cv2.imwrite(str(save_dir.joinpath('images/{:05}.png'.format(i))), frame)
            if i%100 == 0:
...
#skeleton will be checked by terminal temporally
...
for idx in tqdm(range(len(os.listdir(str(img_dir))))):
    ...
    with torch.no_grad():
        ...
        print(heatmap.shape)#(512, 512, 19)
        print(paf.shape)#(512, 512, 38)
